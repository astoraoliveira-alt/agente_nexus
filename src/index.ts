        ...
        try {
            ...
            if (body.direction === 'IN' && body.type === 'MESSAGE') {
                ...
                const { data: agents } = await supabaseAdmin
                    .from('agents')
                    .select('id, name, tenant_id, zenvia_channel_id, whatsapp_provider')
                    .or(`zenvia_channel_id.eq.${channelId},zenvia_aliases.cs.{${channelId}}`)
                    .eq('status', 'active')
                    .limit(1);
                ...
                if (activeProvider !== 'zenvia') {
                    console.log(`[ZENVIA] ⏭️ Skipping Zenvia webhook for channel ${channelId}: agent active provider is '${activeProvider}'`);
                    await logIntegration({
                        provider: 'zenvia',
...
async function startOutboundRecoveryWorker() {
    ...
    const recover = async () => {
        try {
            ...
            if (error) {
                console.error(`[RECOVERY] ❌ Error fetching stuck outbound items: ${error.message} (details: ${JSON.stringify(error)})`);
                return;
            }

            if (!stuckItems || stuckItems.length === 0) return;

            console.log(`[RECOVERY] 🔄 Found ${stuckItems.length} STUCK OUTBOUND messages. Details: ${JSON.stringify(stuckItems.map(i => ({ id: i.id, tenant_id: i.tenant_id, agent_id: i.agent_id, status: i.status, created_at: i.created_at, phone: i.contact_phone }))) || '[]'}`);
            ...
            for (const item of stuckItems) {
                ...
                if (updateError) {
                    console.error(`[RECOVERY] ❌ Failed to update stuck outbound item ID: ${item.id}. Error: ${updateError.message}`);
                } else {
                    console.log(`[RECOVERY] ✅ Successfully re-queued outbound item ID: ${item.id}`);
                }
            }
        } catch (err: any) {
