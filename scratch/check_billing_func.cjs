...
async function run() {
  const { data: def } = await supabase.rpc('execute_sql', { sql_query: "SELECT pg_get_functiondef(oid) as code FROM pg_proc WHERE proname = 'fn_process_whatsapp_billing';" });
  if (def && def.length > 0) console.log(def[0].code);
}
run();
