...
async function inspectConversation() {
  const conversation_id = 'ab78a336-eb39-4f74-8182-3fb3abe7b212';
  ...
  data.forEach(m => {
    console.log(`[${m.created_at}] ${m.sender_type}: ${m.content.substring(0, 30)}... | ExtID: ${m.external_id} | Trace: ${m.trace_id} | Remote: ${m.remote_id}`);
  });
}
