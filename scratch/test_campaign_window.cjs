...
async function run() {
  console.log('Temporarily changing campaign start_date and window to be fully open...');
  
  // 1. Update campaign to open window
  const { error: errUpdateOpen } = await supabase
    .from('campaigns')
    .update({
  ...
  if (errUpdateOpen) {
    console.error('Error updating campaign to open:', errUpdateOpen);
    return;
  }

  console.log('Campaign window opened. Calling get_next_leads_secure...');
  ...
  if (errRpc) {
    ...
    if (leads?.length > 0) {
      console.log('Returned leads detail:', leads.map(l => ({ id: l.id, phone: l.phone })));
    }
  }
