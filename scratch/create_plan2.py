...
Edite o seu fluxo `Agente Nexus - Whatts Fila (FISERV_TICKET) TESTE (4)` para rotear a simulação e chamar a API Fiserv via Subworkflow.
...
### B) Adicionar Nó `Execute Workflow` para Simular Empréstimo
Conecte a nova saída do Switch recém-criada a um novo nó **Execute Workflow**. Configure este nó para chamar o seu subworkflow centralizador (Ferramenta Fiserv Credito):
- **Workflow to call:** `Ferramenta Fiserv Credito`
- **Mode:** `Wait For Workflow To Finish`
- **Data to Send:** `Define Below`
- Crie os seguintes campos nos argumentos da chamada:
  - **action**: `simulate`
  - **loan_request_id**: `{{{{ $('Roteador de Contexto').first().json.lead_info.loan_request_id }}}}`
...
*Dessa forma a autenticação e os headers já estarão devidamente encapsulados no subworkflow oficial.*

### C) Adicionar Nó `Set` para Formatar Resposta
Conecte a saída do nó *Execute Workflow* a um novo nó **Set** (ou Edit Fields). Configure-o para montar a resposta da IA que irá para o WhatsApp:
- Crie um novo campo String chamado `output` (ou o nome da variável de mensagem de saída que você usa no seu fluxo) e coloque a expressão:
```text
Simulação concluída! ✅
...
> [!IMPORTANT]
> O uso do Subworkflow centraliza as regras e simplifica o fluxo! Avalie este plano completo atualizado. Se estiver de acordo, aplique o Javascript e configure os nós no n8n.
"""
