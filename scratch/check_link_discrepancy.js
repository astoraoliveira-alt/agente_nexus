...
async function check() {
    const { data: leads } = await supabase
        .from('agent_leads')
        .select('id, name, whatsapp, cta_link, identifier, campaign_id')
        .or('whatsapp.eq.5511993434870,whatsapp.eq.11993434870');

    console.log('--- AGENT LEADS FOR WHATSAPP 5511993434870 ---');
    leads?.forEach(lead => {
        console.log(`ID: ${lead.id} | Name: ${lead.name} | Phone: ${lead.whatsapp} | CNPJ: ${lead.identifier} | Campaign: ${lead.campaign_id}`);
