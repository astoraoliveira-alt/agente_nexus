...
if (!supabaseUrl || !supabaseServiceKey) {
    console.error('❌ Error: Supabase credentials missing from environment. Please run with --env-file=porteiro/.env');
    process.exit(1);
}
...
async function runDiagnostics() {
    ...
    for (const c of campaigns) {
        ...
        if (total > 0) {
            const statusCounts = queueStats.reduce((acc, curr) => {
            ...
            const missingPhones = queueStats.filter((q) => !q.metadata || (!q.metadata.content && !q.metadata.message)).length;
            const withTemplateId = queueStats.filter((q) => q.metadata?.template_id).length;
