...
async function analyze() {
  for (const camp of campaigns) {
    ...
    const { data: records, error: fetchErr } = await supabase
      .from('outbound_queue')
      .select('sent_at, created_at, scheduled_at')
      .eq('campaign_id', camp.id)
      .not('sent_at', 'is', null)
    ...
    console.log(`- Total Fila: ${totalCount}`);
    console.log(`- Total Enviados (sent_at NOT NULL): ${sentCount}`);
    ...
    console.log(`- Tempo Total de Processamento: ${diffMin.toFixed(2)} minutos`);
    console.log(`- Taxa de Envio: ${msgsPerMin} msgs/min`);
  }
}
