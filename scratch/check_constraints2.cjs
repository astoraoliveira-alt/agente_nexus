...
async function run() {
  const { data: cols, error } = await supabase.rpc('execute_sql', { sql_query: "SELECT conname, pg_get_constraintdef(c.oid) FROM pg_constraint c JOIN pg_namespace n ON n.oid = c.connamespace WHERE c.conrelid = 'whatsapp_billing_windows'::regclass;" });
  if (error) {
    console.error(error);
  } else {
