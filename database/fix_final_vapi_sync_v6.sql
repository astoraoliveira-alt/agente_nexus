-- FIX V6: Dedup + Constraints
-- Purpose: Clean up duplicate logs BEFORE adding the unique constraint

-- 1. Deduplicate integration_logs (Keep only the latest entry)
DELETE FROM integration_logs
WHERE id IN (
    SELECT id
    FROM (
        SELECT id,
        ROW_NUMBER() OVER (partition BY provider, external_id ORDER BY processed_at DESC) as r
        FROM integration_logs
    ) t
    WHERE t.r > 1
);

-- 2. Add Constraint to integration_logs
-- Now it should succeed because duplicates are gone
DROP INDEX IF EXISTS idx_integration_logs_provider_ext;

ALTER TABLE integration_logs 
DROP CONSTRAINT IF EXISTS uq_integration_logs_provider_external_id;

ALTER TABLE integration_logs 
ADD CONSTRAINT uq_integration_logs_provider_external_id 
UNIQUE (provider, external_id);

-- 3. Fix messages constraints (Clean slate)
...
-- 4. Update RPC to use BOTH constraints explicitly
