-- =========================================================================
-- FIX: ISOLAMENTO DE FILA MULTI-TENANT
-- Descrição: Adiciona o parâmetro p_tenant_id à função fn_fetch_next_inbound_message
-- para permitir que múltiplos n8n busquem apenas as mensagens de seus respectivos tenants.
-- =========================================================================

-- 1. Removemos a assinatura antiga para evitar ambiguidade no banco
...
-- 2. Criamos a nova função com o parâmetro opcional
...
BEGIN
    -- [1] Busca Mensagem na Fila (Com filtro isolado de Tenant)
    ...
    WHERE id = (
        ...
        WHERE status = 'pending'
          AND (p_tenant_id IS NULL OR tenant_id = p_tenant_id) -- FILTRO INJETADO AQUI
...
GRANT EXECUTE ON FUNCTION public.fn_fetch_next_inbound_message(INTEGER, TEXT, UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_fetch_next_inbound_message(INTEGER, TEXT, UUID) TO service_role;
