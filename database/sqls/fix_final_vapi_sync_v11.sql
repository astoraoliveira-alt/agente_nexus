-- FIX V11: Debug Mode
-- Purpose: Return internal variable states in the JSON response to diagnose extraction failures.
...
BEGIN
    ...
    -- Try to find 'call' object (Standard vs Flattened)
    ...
    v_debug_info := jsonb_build_object(
        ...
        'message_found', (v_message_obj IS NOT NULL),
        'call_found', (v_call_obj IS NOT NULL),
        'artifact_found', (v_artifact_obj IS NOT NULL),
        'messages_type', jsonb_typeof(v_messages),
        'messages_count_raw', jsonb_array_length(CASE WHEN jsonb_typeof(v_messages) = 'array' THEN v_messages ELSE '[]'::jsonb END),
        'call_id', v_call_id,
        'status', v_status
    );
    ...
    IF v_ended_reason IN ('no-answer', 'busy', 'failed', 'customer-did-not-answer', 'customer-busy', 'voicemail') THEN
        ...
        RETURN jsonb_build_object('success', true, 'status', 'ignored', 'reason', v_ended_reason, 'debug', v_debug_info);
    END IF;
    ...
    -- Extract Usage (Try multiple paths)
