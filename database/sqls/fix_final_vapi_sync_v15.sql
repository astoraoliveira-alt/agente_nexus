-- FIX V15: Explicit Agent ID Parameter
-- Purpose: 
-- 1. Add `p_agent_id` parameter to `sync_vapi_call` to ensure correct agent association.
-- 2. Maintain backward compatibility (DEFAULT NULL).
-- 3. Prioritize `p_agent_id` in agent resolution logic.

-- DROP first to change signature
DROP FUNCTION IF EXISTS sync_vapi_call(UUID, JSONB, VARCHAR, VARCHAR);
