-- FIX V7: Allow 'queued' status
-- Purpose: Relax status check to allow testing/processing of 'queued' calls
...
BEGIN
    ...
    -- We ignore calls that weren't answered or failed, UNLESS it's queued/processing
    ...
    -- Update Duration & Status if ended OR QUEUED
    IF v_status IN ('ended', 'queued', 'ringing', 'in-progress') THEN
        UPDATE conversations 
        SET duration_seconds = v_duration, 
            status = CASE WHEN v_status = 'ended' THEN 'closed' ELSE 'ai_active' END, 
