-- Davos Nexus - Orchestrator V6 with Dynamic Tools Support
-- Extends the Production V5 logic with agent_tools discovery.

DROP FUNCTION IF EXISTS public.n8n_orchestrator_v6(text, text, text, jsonb);
...
BEGIN
    ...
    IF v_agent_id IS NOT NULL THEN
        ...
        -- 2.1 BUSCA DE FERRAMENTAS DINÂMICAS (NOVO V6)
    ...
    RETURN jsonb_build_object(
        ...
        'agent', jsonb_build_object(
            ...
            'tools', COALESCE(v_tools, '[]'::jsonb) -- NOVO V6
        ),
...
GRANT EXECUTE ON FUNCTION public.n8n_orchestrator_v6(text, text, text, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.n8n_orchestrator_v6(text, text, text, jsonb) TO service_role;
