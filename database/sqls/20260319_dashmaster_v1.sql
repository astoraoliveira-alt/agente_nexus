-- =============================================
-- DASHMASTER V1.3: Total Dynamic Costing & Agent Hierarchy
-- Consolida custos dinâmicos (Empresa > Plano) e hierarquia de sub-agentes.
-- =============================================
...
DECLARE
    ...
    -- Preços de Venda (ROI & Selling Prices)
    v_msg_price_venda NUMERIC;
    v_llm_1k_price_venda NUMERIC;
    v_stt_price_venda NUMERIC;
    v_tts_price_venda NUMERIC;
    
    v_operator_hour_rate NUMERIC;
    v_interaction_minutes NUMERIC;
...
BEGIN
    -- 1. CARREGAMENTO DE CONFIGURAÇÕES (Prioridade: Empresa -> Plano -> Default)
    SELECT 
        COALESCE(p.name, 'Flex'),
        COALESCE(c.plan_details->'limits', p.default_limits, '{}'::jsonb),
        -- Preços de Venda dinâmicos (overrides em companies.plan_details)
        COALESCE((c.plan_details->>'message_price')::numeric, p.message_price, 0.1),
        COALESCE((c.plan_details->>'llm_token_price')::numeric, p.llm_token_price, 0.50),
        COALESCE((c.plan_details->>'stt_minute_price')::numeric, p.stt_minute_price, 0.20),
        COALESCE((c.plan_details->>'tts_minute_price')::numeric, p.tts_minute_price, 0.20),
        -- ROI Config dinâmico
        COALESCE((c.roi_config->>'operator_hourly_rate')::numeric, 30.0),
        COALESCE((c.roi_config->>'avg_human_minutes_per_interaction')::numeric, 2.5)
    INTO 
    ...
    -- 2. Estatísticas de Conversa e Incidentes
    ...
    -- 5. Consumo Consolidado (30 dias)
    WITH recorded_metrics AS (
        SELECT metric_type, SUM(value) as val
    ...
    DECLARE
        ...
        v_total_cost_venda NUMERIC := (v_total_msgs * v_msg_price_venda) + 
                                       (v_total_tokens / 1000 * v_llm_1k_price_venda) + 
                                       (v_total_stt * v_stt_price_venda) + 
                                       (v_total_tts * v_tts_price_venda);
    BEGIN
        v_result = jsonb_build_object(
            ...
            'plan', jsonb_build_object(
                ...
                'prices', jsonb_build_object(
                    'message', v_msg_price_venda,
                    'llmToken1k', v_llm_1k_price_venda,
                    'sttMinute', v_stt_price_venda,
                    'ttsMinute', v_tts_price_venda
                )
            ),
            ...
            -- Agentes Agrupados (Sub-agentes somam ao pai no ranking)
            'agents', (
                SELECT jsonb_agg(ag) FROM (
                    WITH agent_stats AS (
                        SELECT 
                            COALESCE(a.parent_agent_id, cm.agent_id) as effective_agent_id,
                        ...
                        WHERE cm.tenant_id = p_tenant_id AND (recorded_at >= v_30_days_ago OR recorded_at IS NULL)
                        GROUP BY 1
                    ),
                    conv_stats AS (
                        SELECT 
                    ...
                    SELECT 
                        a.id, a.name, a.status, a.role, a.type,
                        COALESCE(cs.conv_count, 0) as "totalConversations",
                        CASE WHEN COALESCE(cs.conv_count, 0) > 0 
                            THEN ROUND(COALESCE(ast.msg_count, 0)::numeric / cs.conv_count, 1)
                            ELSE 0 
                        END as "avgMsgsPerUser",
                        jsonb_build_object(
                    ...
                    AND a.parent_agent_id IS NULL -- Apenas agentes topo-de-nível
