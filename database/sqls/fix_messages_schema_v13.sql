-- FIX V13: Schema Update & Metadata Support
-- Purpose: Add missing metadata column to messages table and handle tool roles gracefully.

-- 1. SCHEMA MIGRATION: Add metadata column if it doesn't exist
DO $$
BEGIN
...
BEGIN
    ...
    IF v_messages IS NOT NULL AND jsonb_array_length(v_messages) > 0 THEN
        ...
        LOOP
            ...
            -- Ignore Tool Calls
            IF v_role IN ('tool_calls', 'tool_call_result') THEN
                -- v_skipped_reasons := array_append(v_skipped_reasons, 'Ignored: ' || v_role);
                CONTINUE;
            END IF;

            IF v_role IN ('user', 'bot', 'assistant', 'system', 'function') THEN
                
                -- Map VAPI roles to Agent Nexus roles (ai, user, system)
                IF v_role IN ('bot', 'assistant', 'system', 'function') THEN 
                    v_role := 'ai'; 
                ...
                BEGIN
                    -- NOTE: metadata column must exist for this to work
