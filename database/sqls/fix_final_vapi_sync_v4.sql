-- FIX V4: Procedural Logic (No ON CONFLICT)
-- Purpose: Bypass the constraint issue entirely by checking existence manually

-- 1. Clean up indexes/constraints to be clean
...
-- Create a standard index for fast lookups (NOT UNIQUE CONSTRAINT)
CREATE INDEX IF NOT EXISTS idx_messages_lookup_fast 
ON messages (tenant_id, external_id);

-- 2. Update RPC to use IF NOT EXISTS logic
...
BEGIN
    ...
    -- 8. SYNC MESSAGES (Procedural Loop - No ON CONFLICT)
        ...
        LOOP
            ...
            IF v_role IN ('user', 'bot', 'assistant') THEN
                ...
                SELECT EXISTS(
                    ...
                    AND external_id = (v_call_id || '-' || COALESCE(v_external_order::TEXT, 'msg'))
                ) INTO v_exists;
