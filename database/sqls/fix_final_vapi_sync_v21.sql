...
BEGIN
    ...
    IF v_ended_reason IN ('no-answer', 'busy', 'failed', 'customer-did-not-answer', 'customer-busy', 'voicemail') THEN
        ...
        VALUES (p_tenant_id, 'vapi', v_call_id, p_vapi_payload, 'ignored', 'Call ended reason: ' || COALESCE(v_ended_reason, 'UNKNOWN'));
    ...
    BEGIN
        ...
        IF v_prompt_tokens = 0 AND v_cost_breakdown IS NOT NULL THEN
            -- FIXED BUG: Check if costBreakdown->'llm' is actually a JSON object vs a numeric cost
    ...
    -- 4. FINAL AGENT RESOLUTION (Multiple Fallbacks to catch VAPI dynamic behaviors)
