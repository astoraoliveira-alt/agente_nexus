...
BEGIN
    ...
    BEGIN
        ...
        --           (Removed Agent validation here so it catches manual errors!)
    ...
    -- =============================================
    -- 5. PRE-PROCESS: IGNORE OR FAIL ABORTED CALLS
    -- =============================================
    ...
    IF v_ended_reason IN ('no-answer', 'busy', 'failed', 'customer-did-not-answer', 'customer-busy', 'voicemail') 
       OR (v_ended_reason = 'customer-ended-call' AND v_duration < 10 AND v_is_campaign_call = TRUE) THEN
        ...
        RETURN jsonb_build_object('success', true, 'status', 'ignored', 'reason', v_ended_reason, 'is_campaign', v_is_campaign_call, 'lead_id', v_lead_id, 'debug', v_debug_info);
    END IF;
