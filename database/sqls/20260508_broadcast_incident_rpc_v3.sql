-- SINCRONIZANDO O DISPARO COM CASTS EXPLÍCITOS
...
DECLARE
    ...
    v_lead_uuids UUID[];
BEGIN
    IF p_lead_ids IS NOT NULL THEN v_lead_uuids := p_lead_ids::UUID[]; END IF;

    SELECT * INTO v_incident FROM public.system_incidents 
    ...
    INSERT INTO public.inbound_queue (tenant_id, agent_id, payload, status, priority, message_type, trace_id)
    SELECT p_tenant_id::UUID, v_agent_id,
        jsonb_build_object(
            'name', COALESCE(l.name, '')::TEXT, 
            'phone', COALESCE(l.whatsapp, '')::TEXT,
            'content', (regexp_split_to_array(v_incident.problem_description, '\s+'))[1], 
            'instance', 'broadcast_engine', 'platform', 'system',
            'messageType', 'conversation', 'incident_id', p_incident_id
        ),
        'pending', 10, 'conversation', 'INC-' || encode(gen_random_bytes(4), 'hex')
    FROM public.agent_leads l
    WHERE l.tenant_id = p_tenant_id::UUID AND l.whatsapp IS NOT NULL
      AND ((v_lead_uuids IS NOT NULL AND l.id = ANY(v_lead_uuids)) OR (v_lead_uuids IS NULL AND (v_incident.campaign_id IS NULL OR l.campaign_id = v_incident.campaign_id)))
      AND NOT EXISTS (
