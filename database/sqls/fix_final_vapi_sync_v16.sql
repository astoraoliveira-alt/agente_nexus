-- FIX V16: Normalize Phone Number Identifier
-- Purpose: 
-- 1. Strip non-numeric characters from user identifier to prevent duplicates (e.g. +5511 vs 5511).
-- 2. Ensure consistency in conversation matching.

-- DROP first to change signature if needed (keeping same signature as V15)
...
BEGIN
    ...
    -- Normalize: Remove all non-numeric characters from the identifier if it looks like a phone number
    ...
    IF p_user_identifier IS NOT NULL AND p_user_identifier != '' THEN
        ...
        -- If stripping resulted in empty (e.g. it was just a name), fallback to original
