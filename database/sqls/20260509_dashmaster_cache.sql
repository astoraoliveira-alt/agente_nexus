-- Migração: Dashmaster Cache System (Quick Win)
-- Objetivo: Reduzir carga no banco ao cachear KPIs do Dashboard por 30 segundos.
...
-- 2. Atualização da RPC get_dashmaster_v1 para usar o cache
...
DECLARE
    ...
    v_interaction_minutes NUMERIC := 2.0;
    ...
    v_limits JSONB;
BEGIN
    -- [A] Tenta ler do cache (TTL 30s)
    SELECT cached_data, updated_at INTO v_cached_data, v_updated_at 
    FROM public.dash_cache 
    WHERE tenant_id = p_tenant_id;

    IF v_updated_at IS NOT NULL AND v_updated_at > (NOW() - INTERVAL '30 seconds') THEN
        RETURN v_cached_data;
    END IF;

    -- [B] Se cache expirado ou inexistente, calcula (Lógica Original V1.2)
            ...
            'agents', (
                SELECT jsonb_agg(ag) FROM (
                    ...
                    SELECT 
                        ...
                        jsonb_build_object('totalMessages', COALESCE(ast.msg_count, 0)) as usage
                    FROM agents a
                    LEFT JOIN agent_stats ast ON ast.effective_agent_id = a.id
                    LEFT JOIN conv_stats cs ON cs.effective_agent_id = a.id
                    WHERE a.tenant_id = p_tenant_id AND a.parent_agent_id IS NULL
    ...
    -- [C] Atualiza o cache e retorna
    INSERT INTO public.dash_cache (tenant_id, cached_data, updated_at)
    VALUES (p_tenant_id, v_result, NOW())
    ON CONFLICT (tenant_id) DO UPDATE SET cached_data = v_result, updated_at = NOW();
