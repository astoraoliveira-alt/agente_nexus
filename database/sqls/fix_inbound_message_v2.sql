-- ======================================================== --
-- DAVOS NEXUS - FIX RPC (N8N Payload compatibility)       --
-- Mapeamento Rigoroso e Prompt Dinâmico                   --
-- ======================================================== --

-- Drop explícito para limpar cache de plano do Postgres, se aplicável
DROP FUNCTION IF EXISTS public.fn_fetch_next_inbound_message(INT, TEXT);
...
DECLARE
    ...
    -- Usando variáveis individuais para não depender de SELECT * e evitar erro de field not found
    v_conv_status VARCHAR;
    v_conv_context_state JSONB;
    v_conv_reopened_at TIMESTAMPTZ;
...
BEGIN
    ...
    -- Busca explícita para evitar o erro "has no field reopened_at" caso schema fique no cache do N8N
    SELECT status, context_state, reopened_at 
    INTO v_conv_status, v_conv_context_state, v_conv_reopened_at
    ...
    -- Fallbacks para o caso de usarem snake_case internamente
    ...
    -- [E] Busca Histórico (Alterado APENAS para isolar pela data de Reabertura)
    SELECT jsonb_agg(h) INTO v_history FROM (
        ...
        FROM (
            ...
            WHERE conversation_id = v_record.conversation_id 
              -- MÁGICA DE ISOLAMENTO:
              AND (v_conv_reopened_at IS NULL OR created_at >= v_conv_reopened_at)
            ORDER BY created_at DESC 
            -- Mude de 10 para 16 aqui se quiser mais mensagens
            LIMIT 10
        ) sub
    ...
    -- [I] CONSTRUÇÃO DO RETORNO (IDÊNTICO AO ORIGINAL, MANTENDO TODAS AS CHAVES DO N8N)
    v_retorno := jsonb_build_object(
        ...
        'agent', jsonb_build_object(
            ...
            'brain_config', v_mod_brain_config, -- O Segredo do Prompt Mágico
        ...
        'conversation', jsonb_build_object(
            ...
            'status', COALESCE(v_conv_status, 'ai_active'), 
            'context_state', COALESCE(v_conv_context_state, '{}'::jsonb),
            'reopened', COALESCE(v_conv_status = 'closed', FALSE)
        ),
