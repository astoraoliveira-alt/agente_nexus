...
BEGIN
    ...
    BEGIN
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        v_is_campaign_call := FALSE;
    END;

    -- FIX: Extract Timestamps and Duration (VAPI sends durationSeconds now)
    BEGIN
        v_duration := COALESCE(
    ...
    -- =============================================
    -- PRE-PROCESS: IGNORE OR FAIL ABORTED CALLS
    -- =============================================
    -- Se nao atendeu (no-answer), falhou a ligacao (failed)
    -- OU se atendeu mas durou muito pouco (customer-ended-call < 10s pode ser considerado falha/desligou na cara)
    IF v_ended_reason IN ('no-answer', 'busy', 'failed', 'customer-did-not-answer', 'customer-busy', 'voicemail') 
       OR (v_ended_reason = 'customer-ended-call' AND v_duration < 10 AND v_is_campaign_call = TRUE) THEN
        ...
        -- Atualizar Fila de Disparo (Campaigns) como FAILED para retentativa no n8n se necessario
        ...
        VALUES (p_tenant_id, 'vapi', v_call_id, p_vapi_payload, 'ignored', 'Call ended/failed reason: ' || COALESCE(v_ended_reason, 'UNKNOWN') || ' (' || v_duration || 's)');
        
        RETURN jsonb_build_object('success', true, 'status', 'ignored', 'reason', v_ended_reason, 'is_campaign', v_is_campaign_call);
    END IF;
    
    -- CAMPAIGN SUCCESS DETECTION (If made it past the failed filters)
    IF v_is_campaign_call = TRUE AND v_status IN ('ended', 'completed') THEN
        BEGIN
            UPDATE outbound_queue 
            SET status = 'sent',
    ...
    -- 4. FINAL AGENT RESOLUTION
    IF p_agent_id IS NOT NULL THEN
        v_agent_id := p_agent_id;
    ELSE
        -- 4.1 Try Server Headers sent by Vapi to N8N (as seen in payloads)
        v_agent_id := COALESCE(
        ...
        -- 4.2 Try Metadata parameter
        IF v_agent_id IS NULL THEN
            v_agent_id := (v_assistant_obj->'metadata'->>'agent_id')::UUID;
        END IF;
        
        -- 4.3 Try VAPI Assistant ID Database mapping
        ...
        -- 4.4 Fallback: First Agent in DB (LAST RESORT)
        IF v_agent_id IS NULL THEN
            SELECT id INTO v_agent_id FROM agents WHERE tenant_id = p_tenant_id LIMIT 1;
        END IF;
    END IF;

    -- 5. RESOLVE CONTACT IDENTIFIER (Hybrid + Normalization)
    ...
    IF v_recording_url IS NOT NULL THEN
        -- Insert or Update conversation artifact tracking to be platform agnostic
        BEGIN
    ...
    -- 7. RECORD CONSUMPTION (ULTRA SAFE FALLBACK STRATEGY)
    IF v_status IN ('ended', 'queued', 'in-progress') THEN
        BEGIN
            ...
            IF v_total_tokens > 0 THEN
                ...
                BEGIN
                    -- Priority 1: Try record_usage with 6 parameters (p_tenant_id included)
                    PERFORM record_usage(p_tenant_id, v_agent_id::TEXT, 'tokens'::TEXT, v_total_tokens::NUMERIC, v_token_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'llm_sync'));
                EXCEPTION WHEN OTHERS THEN
                    BEGIN
                        -- Priority 2: Try record_usage with 5 parameters (Original signature)
                    ...
                    EXCEPTION WHEN OTHERS THEN
                        v_skipped_reasons := array_append(v_skipped_reasons, 'rpc[tokens] failed: ' || SQLERRM);
                        -- Priority 3: Fallback direct insertion so we don't lose the consumption data
                        INSERT INTO consumption_metrics (tenant_id, agent_id, channel, metric_type, value, cost, metadata) 
                        VALUES (p_tenant_id, v_agent_id, 'voice'::conversation_channel, 'tokens'::metric_type, v_total_tokens::NUMERIC, v_token_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'llm_sync', 'failed_rpc', true));
                    END;
                END;
            END IF;
            ...
            IF v_duration > 0 THEN
                ...
                BEGIN
                    PERFORM record_usage(p_tenant_id, v_agent_id::TEXT, 'stt_minutes'::TEXT, (v_duration::NUMERIC / 60.0), v_stt_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'stt_sync'));
                EXCEPTION WHEN OTHERS THEN
                    BEGIN
                    ...
                    EXCEPTION WHEN OTHERS THEN
                        v_skipped_reasons := array_append(v_skipped_reasons, 'rpc[stt] failed: ' || SQLERRM);
                        INSERT INTO consumption_metrics (tenant_id, agent_id, channel, metric_type, value, cost, metadata) 
                        VALUES (p_tenant_id, v_agent_id, 'voice'::conversation_channel, 'stt_minutes'::metric_type, (v_duration::NUMERIC / 60.0), v_stt_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'stt_sync', 'failed_rpc', true));
                    END;
                END;

                v_tts_cost := (v_duration::NUMERIC / 60.0) * v_plan_tts_price;
                BEGIN
                    PERFORM record_usage(p_tenant_id, v_agent_id::TEXT, 'tts_minutes'::TEXT, (v_duration::NUMERIC / 60.0), v_tts_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'tts_sync'));
                EXCEPTION WHEN OTHERS THEN
                    BEGIN
                    ...
                    EXCEPTION WHEN OTHERS THEN
                        v_skipped_reasons := array_append(v_skipped_reasons, 'rpc[tts] failed: ' || SQLERRM);
                        INSERT INTO consumption_metrics (tenant_id, agent_id, channel, metric_type, value, cost, metadata) 
                        VALUES (p_tenant_id, v_agent_id, 'voice'::conversation_channel, 'tts_minutes'::metric_type, (v_duration::NUMERIC / 60.0), v_tts_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'tts_sync', 'failed_rpc', true));
                    END;
                END;
            END IF;
