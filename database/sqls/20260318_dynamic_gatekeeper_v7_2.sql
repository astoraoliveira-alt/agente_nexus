-- Correção de Estrutura de Retorno para n8n (Remapeamento Definitivo v7_2)
-- Compatibilidade 100% estrita com os campos esperados pelo Nó "Edit Fields" no N8N
...
BEGIN
    ...
    IF v_parent_requires_security AND COALESCE(v_session_status, 'unauthenticated') != 'active' THEN
        ...
        END IF;
    ELSE
        -- Quando autenticado (active) ou sem requisito de segurança, volta pro agente pai
        v_active_agent_id := v_parent_agent_id;
    END IF;

    -- Grava a nova identidade no banco para tracking
    ...
    -- BUSCA DE SUB-AGENTES DO PAI (Para evitar nulo no n8n Edit Fields > sub_agents)
    SELECT jsonb_agg(jsonb_build_object(
        ...
        'modelId', brain_config->>'modelId', 
        'temperature', (brain_config->>'temperature')::NUMERIC
    ))
    INTO v_sub_agents FROM public.agents WHERE parent_agent_id = v_parent_agent_id;
    ...
    SELECT jsonb_agg(tool_sub) INTO v_tools FROM (
        ...
        WHERE tenant_id = v_tenant_id AND (agent_id = v_agent_id OR agent_id IS NULL) AND is_active = TRUE
    ) tool_sub;

    -- 8. RETORNO CONSOLIDADO (MATCH EXATO DO 'EDIT FIELDS' v6_final)
    RETURN jsonb_build_object(
        ...
        'governance', jsonb_build_object(
            ...
            'qnt_transacoes_correntes', v_active_conv_count,  -- RESTAURADO!
            'max_concurrency', v_max_concurrency,             -- RESTAURADO!
            'lifecycle_stage', v_lifecycle_stage,             -- RESTAURADO!
            'autonomy_level', v_autonomy_level,               -- RESTAURADO!
        ...
        'agent', jsonb_build_object(
            ...
            'brain_config', v_agent_config,                   -- RESTAURADO!
        ...
        'conversation', jsonb_build_object(
            ...
            'reopened', v_reopened                            -- RESTAURADO!
        ),
        'security', jsonb_build_object(                       -- RESTAURADO TOTALMENTE IDENTICO
