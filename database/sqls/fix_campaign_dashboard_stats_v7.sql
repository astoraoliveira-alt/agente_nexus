...
BEGIN
  ...
  SELECT 
    ...
    -- Enviados: Leads que tiveram sucesso no disparo (ignora falhas e pendentes)
    COUNT(*) FILTER (
      WHERE actual_status IN ('sent', 'enviada', 'delivered', 'entregue', 'read', 'lida', 'converted', 'convertida') 
         OR response_detected = true
    ) AS sent,
    
    -- Entregues: Conforme regra de negócio, reflete o mesmo número de enviados 
    -- (assume-se que se a Zenvia despachou, foi entregue, a menos que volte erro)
    COUNT(*) FILTER (
      WHERE actual_status IN ('sent', 'enviada', 'delivered', 'entregue', 'read', 'lida', 'converted', 'convertida') 
         OR response_detected = true
    ) AS delivered,
    
    -- Lidas: Apenas confirmadas por status de leitura ou interação
    COUNT(*) FILTER (
      WHERE actual_status IN ('read', 'lida', 'converted', 'convertida') 
         OR response_detected = true
    ) AS read_count,
    
    -- Interagiram: Interações explícitas
  ...
  -- 4. Cálculo de Conversão (Sucesso / Links)
