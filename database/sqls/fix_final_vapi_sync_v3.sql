-- FIX V3: Explicit Constraint Strategy
-- Purpose: Remove index ambiguity by using a named CONSTRAINT

-- 1. Clean up ALL confusion (Drop old/partial indexes)
DROP INDEX IF EXISTS idx_messages_conversation_order; -- The partial one found in logs
DROP INDEX IF EXISTS idx_messages_idempotency; -- The one from migration
DROP INDEX IF EXISTS idx_messages_tenant_external_id; -- The one from V2

-- 2. Add Explicit Constraint
-- This ensures there is absolutely no ambiguity for ON CONFLICT
ALTER TABLE messages 
DROP CONSTRAINT IF EXISTS uq_messages_tenant_external_id;

ALTER TABLE messages 
ADD CONSTRAINT uq_messages_tenant_external_id 
UNIQUE (tenant_id, external_id);

-- 3. Update RPC to use ON CONSTRAINT
