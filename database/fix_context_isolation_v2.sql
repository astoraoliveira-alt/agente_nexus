-- =========================================================================
-- Davos Nexus - Context Isolation FIX (V2.7) - FINAL PRODUCTION SYNC
-- Protocol: Injeção de Workflow Blueprint no Código de Produção Original
-- =========================================================================

-- 1. Limpeza de overloads conflitantes (Prevenção de erro PGRST203)
DROP FUNCTION IF EXISTS public.record_message(uuid, uuid, text, text, text, text, text, text, jsonb, uuid);
DROP FUNCTION IF EXISTS public.record_message(uuid, uuid, text, text, text, text, text, jsonb);
DROP FUNCTION IF EXISTS public.fn_fetch_next_inbound_message(text);
DROP FUNCTION IF EXISTS public.fn_fetch_next_inbound_message(integer, text);
DROP FUNCTION IF EXISTS public.fn_close_conversation(uuid);
DROP FUNCTION IF EXISTS public.close_conversation(uuid);
DROP FUNCTION IF EXISTS public.fn_reset_conversation(uuid);

-- 2. Colunas de isolamento e Trigger
ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS reopened_at TIMESTAMPTZ;
ALTER TABLE public.agents ADD COLUMN IF NOT EXISTS workflow_blueprint JSONB DEFAULT NULL;

CREATE OR REPLACE FUNCTION trg_set_conversation_reopened_at()
RETURNS trigger AS $$
BEGIN
    IF OLD.status = 'closed' AND NEW.status IN ('ai_active', 'human_active') THEN
        NEW.reopened_at = NOW();
    END IF;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_conversations_reopened ON public.conversations;
CREATE TRIGGER trg_conversations_reopened
BEFORE UPDATE ON public.conversations
FOR EACH ROW
EXECUTE FUNCTION trg_set_conversation_reopened_at();

-- 3. Função de Reset Manual
CREATE OR REPLACE FUNCTION public.fn_reset_conversation(id UUID)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    UPDATE public.conversations 
    SET reopened_at = NOW(), status = 'ai_active', updated_at = NOW()
    WHERE public.conversations.id = fn_reset_conversation.id;
    RETURN jsonb_build_object('status', 'success', 'message', 'Histórico isolado');
END;
$$;

-- 4. record_message (Produção + Reset Hook)
...
BEGIN
    IF trim(p_content) = '#reset' THEN PERFORM public.fn_reset_conversation(p_conversation_id); END IF;
    INSERT INTO messages (conversation_id, tenant_id, content, sender_type, sender_name, message_type, trace_id, metadata)
    VALUES (p_conversation_id, p_tenant_id, public.clean_message_content(p_content), p_sender_type, p_sender_name, p_message_type, p_trace_id, p_metadata);
    UPDATE public.conversations SET updated_at = NOW(), last_message_at = NOW() WHERE id = p_conversation_id;
    RETURN jsonb_build_object('status', 'success');
END;
$$;

-- 5. close_conversation (Suporte a JSON {"id":...})
CREATE OR REPLACE FUNCTION public.close_conversation(id UUID)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    UPDATE public.conversations SET status = 'closed', updated_at = NOW() WHERE public.conversations.id = close_conversation.id;
    RETURN jsonb_build_object('status', 'success');
END;
$$;

-- 6. fn_close_conversation (Alias)
CREATE OR REPLACE FUNCTION public.fn_close_conversation(id UUID)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    RETURN public.close_conversation(fn_close_conversation.id);
END;
$$;

-- 7. fn_fetch_next_inbound_message (CÓDIGO INTEGRAL DE PRODUÇÃO + NOVOS CAMPOS)
CREATE OR REPLACE FUNCTION public.fn_fetch_next_inbound_message(p_n8n_execution_id TEXT DEFAULT NULL)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    -- [MOD] Histórico com Isolamento
    SELECT jsonb_agg(h) INTO v_history FROM (
        SELECT sender_type, content, created_at FROM (
            SELECT sender_type, content, created_at FROM public.messages 
    ...
    v_retorno := jsonb_build_object(
        ...
        'agent', jsonb_build_object(
            ...
            -- [NOVO] Adição do Blueprint de Workflow
...
-- 8. Persistência de Estado (State Machine)
ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS context_state JSONB DEFAULT '{}'::jsonb;
...
BEGIN
    ...
    RETURN jsonb_build_object('status', 'success', 'current_step', p_current_step);
END;
$$;
