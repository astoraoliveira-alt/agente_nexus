-- =============================================
-- FIX V17: Generic Cost Tracking and Artifact Sync
-- Purpose: 
-- 1. Strip non-numeric characters from user identifier to prevent duplicates.
-- 2. Store platform_cost and platform_cost_breakdown in conversation metadata generically.
-- 3. Insert audio recordings into conversation_artifacts table upon sync.
-- =============================================

-- DROP first to change signature if needed (keeping same signature as V15/V16)
DROP FUNCTION IF EXISTS sync_vapi_call(UUID, JSONB, VARCHAR, VARCHAR, UUID);
...
BEGIN
    ...
    v_platform_cost := COALESCE((v_message_obj->>'cost')::NUMERIC, (v_call_obj->>'cost')::NUMERIC);
    ...
    IF v_recording_url IS NOT NULL THEN
        ...
        EXCEPTION WHEN OTHERS THEN 
            RAISE NOTICE 'Skipping artifact insertion due to error: %', SQLERRM;
        END;
    END IF;
