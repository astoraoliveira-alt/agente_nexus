-- FIX V12: Role Normalization & Debug Skipped
-- Purpose: Fix potential case-sensitivity issues in 'role' and debug why messages are skipped.
...
BEGIN
    ...
    -- BUILD DEBUG INFO (Dynamic update later)
    ...
    -- 8. SYNC MESSAGES (Normalized & Robust)
    IF v_messages IS NOT NULL AND jsonb_array_length(v_messages) > 0 THEN
        ...
        LOOP
            ...
            IF v_role IN ('user', 'bot', 'assistant', 'system', 'tool', 'function') THEN
                
                -- Map VAPI roles to Agent Nexus roles (ai, user, system)
                IF v_role IN ('bot', 'assistant', 'system', 'tool', 'function') THEN 
                    v_role := 'ai'; 
    ...
    v_debug_info := v_debug_info || jsonb_build_object(
        ...
        'sample_raw_role', (v_messages->0->>'role')
    );
