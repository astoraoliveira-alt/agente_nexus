CREATE OR REPLACE FUNCTION n8n_orquestrator_v4(
...
BEGIN
    ...
    INTO v_agent_id, v_tenant_id, v_agent_name, v_agent_status, v_max_concurrency, 
         ...
         v_applied_policies, v_integration_config, v_whatsapp_api_type, v_meta_api_token
    FROM agents WHERE evolution_instance = p_instance_name LIMIT 1;

    -- 2. Busca de Sub-agentes (Filhos)
    IF v_agent_id IS NOT NULL THEN
        ...
        FROM agents 
        WHERE parent_agent_id = v_agent_id;
    END IF;
    ...
    IF v_agent_id IS NOT NULL THEN
        ...
        -- SISTEMA DAVOS DE PESOS (0.3 por palavrão, 3.0 para banir)
    ...
    -- Retorno de Bloqueio (Mantendo todos os campos para o n8n)
    IF NOT v_agente_encontrado OR NOT v_agente_ativo OR NOT v_empresa_ativa OR v_limite_atingido OR v_user_banned THEN
        RETURN jsonb_build_object(
            ...
            'governance', jsonb_build_object(
                ...
                'incident_count', v_risk_score, -- Passando o score para fins de log
    ...
    -- 3. Políticas e Regras de Governança
    ...
    -- 4. Recuperação/Criação da Conversa
    ...
    -- 5. Contexto Dinâmico (Histórico e RAG)
    ...
    -- 6. Lógica de Segurança (Identity Gate / Sessão)
    ...
    ELSIF v_identity_gate_enabled THEN
        v_system_prompt := v_system_prompt || E'\n\n[SISTEMA DE SEGURANÇA GATEKEEPER]: REGRA: PROIBIDO pedir documentos agora.';
    END IF;

    -- 7. Retorno Final Robusto (Sem Undefined no n8n)
    RETURN jsonb_build_object(
        'status', 'success',
        'governance', jsonb_build_object(
