...
BEGIN
    ...
    SELECT 
        ...
        ROUND((
            (COALESCE(rm.val_llm, 0) / 1000.0 * COALESCE(p.llm_token_price, 0.02)) + 
            -- Voice: Sum STT and TTS revenue (each is approx 0.50 if plan is 1.00 total)
            (COALESCE(rm.val_stt, 0) * COALESCE(p.stt_minute_price, 0.50)) + 
            (COALESCE(rm.val_tts, 0) * COALESCE(p.tts_minute_price, 0.50)) +
            -- Messages: Use the maximum between metric record and message table count to avoid doubling
            (GREATEST(COALESCE(rm.val_msgs_recorded, 0), COALESCE(mc.total_msgs, 0)) * COALESCE(p.message_price, 0))
