-- FIX: Add UNIQUE Constraint to integration_logs and restore messages constraint
-- Purpose: Fix the "no unique constraint matching ON CONFLICT" error in integration_logs

-- 1. Fix integration_logs (The hidden culprit)
-- It had a normal index but needs a UNIQUE constraint for ON CONFLICT to work
DROP INDEX IF EXISTS idx_integration_logs_provider_ext;

ALTER TABLE integration_logs 
ADD CONSTRAINT uq_integration_logs_provider_external_id 
UNIQUE (provider, external_id);

-- 2. Restore messages constraint (clean slate)
-- We want to go back to declarative logic if possible, now that we know the real error was elsewhere
DROP INDEX IF EXISTS idx_messages_conversation_order;
DROP INDEX IF EXISTS idx_messages_idempotency;
DROP INDEX IF EXISTS idx_messages_tenant_external_id;
DROP INDEX IF EXISTS idx_messages_lookup_fast;
ALTER TABLE messages DROP CONSTRAINT IF EXISTS uq_messages_tenant_external_id;

-- Add the DEFINITIVE constraint for messages too
ALTER TABLE messages 
ADD CONSTRAINT uq_messages_tenant_external_id 
UNIQUE (tenant_id, external_id);


-- 3. Update RPC to use BOTH constraints explicitly
...
BEGIN
    ...
    -- 3. AUDIT LOG (USE CONSTRAINT EXPLICITLY)
    ...
    -- 8. SYNC MESSAGES (Declarative Loop - ON CONSTRAINT)
        ...
        LOOP
            ...
            IF v_role IN ('user', 'bot', 'assistant') THEN
                ...
                BEGIN
                    ...
                    -- Explicitly naming the constraint to avoid ANY ambiguity
...
EXCEPTION WHEN OTHERS THEN
    -- Fallback audit (try/catch update)
    BEGIN
