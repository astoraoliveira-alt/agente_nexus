...
BEGIN
    ...
    BEGIN
        ...
        IF v_prompt_tokens = 0 AND v_cost_breakdown IS NOT NULL THEN
            IF v_cost_breakdown->'llm' IS NOT NULL THEN
        ...
        IF v_completion_tokens = 0 AND v_cost_breakdown IS NOT NULL THEN
            IF v_cost_breakdown->'llm' IS NOT NULL THEN
    ...
    -- 4. RESOLVE AGENT (Priority: Explicit Param > Assistant ID > Fallback)
    IF p_agent_id IS NOT NULL THEN
        v_agent_id := p_agent_id;
    ELSE
        -- Try Metadata in Payload
        v_agent_id := (v_assistant_obj->'metadata'->>'agent_id')::UUID;
        
        -- Try VAPI Assistant ID mapping
        ...
        -- Fallback: Any agent (LAST RESORT)
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        UPDATE integration_logs SET status = 'error', error_details = 'Failed to create conversation: ' || SQLERRM WHERE provider = 'vapi' AND external_id = v_call_id;
    ...
    -- NOTE: record_usage takes UUID for p_agent_id, text for p_metric_type!
    IF v_status IN ('ended', 'queued', 'in-progress') THEN
        BEGIN
            IF v_total_tokens > 0 THEN
                ...
                PERFORM record_usage(p_tenant_id, v_agent_id, 'tokens'::TEXT, v_total_tokens::NUMERIC, v_token_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'llm_sync'));
            END IF;

            IF v_duration > 0 THEN
                v_stt_cost := (v_duration::NUMERIC / 60.0) * v_plan_stt_price;
                PERFORM record_usage(p_tenant_id, v_agent_id, 'stt_minutes'::TEXT, (v_duration::NUMERIC / 60.0), v_stt_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'stt_sync'));
                v_tts_cost := (v_duration::NUMERIC / 60.0) * v_plan_tts_price;
                PERFORM record_usage(p_tenant_id, v_agent_id, 'tts_minutes'::TEXT, (v_duration::NUMERIC / 60.0), v_tts_cost, jsonb_build_object('vapi_call_id', v_call_id, 'type', 'tts_sync'));
            END IF;
        EXCEPTION WHEN OTHERS THEN
            v_skipped_reasons := array_append(v_skipped_reasons, 'Record usage error: ' || SQLERRM);
        END;
    END IF;
