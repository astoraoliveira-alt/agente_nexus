-- =========================================================================
-- Davos Nexus - Context Isolation (Caminho 2)
-- Adiciona corte temporal no histórico sem duplicar conversas na UI
-- Mantém 100% da sua estrutura original de payload
-- =========================================================================

-- 1. Adicionar o marco temporal na tabela de conversas
ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS reopened_at TIMESTAMPTZ;

-- 2. Criar Trigger para preencher o marco sempre que for reaberta
CREATE OR REPLACE FUNCTION trg_set_conversation_reopened_at()
RETURNS trigger AS $$
BEGIN
    -- Se o status antes era 'closed' e a conversa voltou para ativa:
...
-- =========================================================================
-- 3. Atualiza sua RPC EXATAMENTE como estava (fn_fetch_next_inbound_message)
-- =========================================================================
...
BEGIN
    ...
    -- [I] CONSTRUÇÃO DO RETORNO (IDÊNTICO AO SEU)
...
-- =========================================================================
-- 4. Atualiza sua RPC EXATAMENTE como estava (get_agent_context)
-- =========================================================================
...
BEGIN
    -- Busca a data de reabertura da conversa
    SELECT reopened_at INTO v_reopened_at
    ...
    -- Get Recent Messages (Alterado APENAS para isolar pela data de Reabertura)
    ...
    -- Return Combined (IDÊNTICO AO SEU)
